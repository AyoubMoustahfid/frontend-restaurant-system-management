...
import {getFournisseurById, getAllFruitByFournisseur, deleteFruit} from "./../ApiFournisseur"
...
function FruitDashboard() {
    ...
    const [fruitUpdate, setFruiUpdate] = useState("")
    ...
    const handleChange = (e) => {
        ...
        setFruits({...fruits , [e.target.id] : value})
    } 

    const handleUpdateFruit = (e) => {
        const value = e.target.id === 'photo' ? e.target.files[0] : e.target.value;
        formData.set(e.target.id, value)
        setFruiUpdate({...fruitUpdate , [e.target.id] : value})
    }
    // *********** CREATE FRUIT *****************
    const submitFruit = e  => {
        e.preventDefault()
        
        fetch(`${API_URL}/fruit/create/${user._id}`, {
            method: 'POST',
            headers: {
        ...
        .then(res => {
            ...
            toastr.success(`Fruit ${fruits.name} created`, 'new Fruit', {
                positionClass: "toast-bottom-left",
            })
    ...
    //*********** GET FRUIT BY ID ************
    const fruitById = async  (id) => {
        await fetch(`${API_URL}/fruit/${id}`, {
           method: 'GET',
           headers: {
          ...
          .then( res => {
              localStorage.setItem("fruit", JSON.stringify(res.fruit))
               setFruitId(res.fruit)
      ...
      const updateFruit = async  (e) => {
        ...
        const {user, token} = isAuthenticated();
        const fruit = await JSON.parse(localStorage.getItem('fruit'))
        console.log(fruit._id);
        fetch(`${API_URL}/fruit/update/${fruit._id}/${user._id}`, {
            method: "PUT",
            headers: {
                "Accept": "application/json",
                "Authorization": `Bearer ${token}`
    ...
    useEffect(() => {
        getFournisseurById(user._id, token)
            ...
            getAllFruitByFournisseur(user._id, token)
                .then(res => {
                    setElements(res.fruit)
                    setPaginationFruits(lodash(res.fruit).slice(0).take(PageSize).value())
                }).catch(err => console.error(err, 'Server error !'))
    }, [])
        ...
        <Layout >
            <div className="row px-3">
                <div className="col-12">
                    ...
                    <div className="row align-items-center p-2 bg-white my-2">
                        <div className="col-6">
                            <h2 className="page-heading">Hi,Welcome Back!</h2>
                            <p className="mb-0">Gestion Fruit in your store</p>
                        </div>
                                    ...
                                    <div className="modal-body">
                                        <form onSubmit={submitFruit}> 
                                            ...
                                            <div className="mb-3 text-start">
                                                ...
                                                <select value={fruits.fournisseur} onChange={handleChange} name="fournisseur" id="fournisseur" className="form-control">
                ...
                <table className="table table-bordered">
                        ...
                        <tbody>
                            ...
                            {paginateFruits && paginateFruits.map((item, i) => (
                                <tr key={item._id}>
                                    <th >
                                        <img  src={`${API_URL}/fruit/photo/${item._id}`} alt={`${item.name}`} width="100px" />
                                    </th>
                                    ...
                                    <td>
                                        <div className="d-flex justify-content-evenly">
                                            <button className="btn btn-primary" data-bs-toggle="modal" data-bs-target="#updateFruit" onClick={() => fruitById(item._id)}> 
                                            ...
                                            <div className="modal fade" id="updateFruit" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden={active}>
                                                <div className="modal-dialog">
                                                    <div className="modal-content">
                                                    <div className="modal-header">
                                                        <h5 className="modal-title" id="exampleModalLabel">Modal title</h5>
                                                    ...
                                                    <div className="modal-body">
                                                        <form onSubmit={updateFruit}> 
                                                            ...
                                                            <div className="mb-3">
                                                                <label htmlFor="name" className="form-label">Name : </label>
                                                                <input type="text" className="form-control" id="name" name="name" required onChange={handleUpdateFruit} defaultValue={fruitId &&  fruitId.name}/>
                                                            </div>

                                                            <div className="mb-3">
                                                                <label htmlFor="price" className="form-label">Price :</label>
                                                                <input type="number" className="form-control" id="price" name="price" required onChange={handleUpdateFruit} defaultValue={fruitId &&  fruitId.price}/>
                                                            </div>

                                                            <div className="mb-3">
                                                                <label htmlFor="quantity" className="form-label">Quantity : </label>
                                                                <input type="number" className="form-control" id="quantity" name="quantity" required onChange={handleUpdateFruit} defaultValue={fruitId &&  fruitId.quantity}/>
                                                            </div>

                                                            <div className="mb-3">
                                                                <label htmlFor="package" className="form-label">Package : </label>
                                                                <input type="number" className="form-control" id="package" name="package" required onChange={handleUpdateFruit} defaultValue={fruitId &&  fruitId.package}/>
                                                            </div>
                                            ...
                                            <button className="btn btn-danger" onClick={() => deleteFruit(item._id, user._id, token).then(res => window.location.reload())}>
